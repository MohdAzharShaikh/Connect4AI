...
export const isDraw = (gameBoard, currentMove, currentPlayer) => {
  let board = [...gameBoard]; // Copy the board
  board[currentMove] = currentPlayer; // Simulate the current move

  let count = board.reduce((n, x) => n + (x === 0), 0); // Count empty spots

  console.log(`count ${count}`);
  return count === 0; // If no empty circles, it's a draw
}
...
const getRandomComputerMove = (gameBoard) => {
  ...
  if (validMoves.length === 0) return -1; // or null, or throw an error

  const rndMove = Math.floor(Math.random() * validMoves.length);
  return validMoves[rndMove];
};


// Helper function to analyze potential winning/blocking positions
...
export const getComputerMove = (gameBoard) => {
  ...
  return getRandomComputerMove(gameBoard); // fallback
};
